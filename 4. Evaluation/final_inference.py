...
def main():
    ...
    print(f"  [OK] Predictions completed: {len(y_pred)} samples")
    
    print("Predicting probabilities...")
    y_proba = model.predict_proba(X_unlabeled)
    print(f"  [OK] Probabilities completed: {y_proba.shape}")
    
    # Get confidence (max probability)
    confidence = y_proba.max(axis=1)
    print(f"  [OK] Confidence scores extracted")
    ...
    print(f"  [OK] Combined {len(final_results)} results")
    ...
    print(f"  [OK] Saved JSON to: {FINAL_PREDICTIONS_FILE}")
    ...
    print(f"  [OK] Saved CSV to: {FINAL_PREDICTIONS_CSV}")
