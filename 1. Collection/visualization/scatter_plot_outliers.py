...
def get_data():
    ...
    try:
        ...
        print(f"[OK] Lấy {len(documents)} documents\n")
        client.close()
        return documents
    except Exception as e:
        print(f"[FAIL] Lỗi: {e}")
...
def create_scatter_chart(documents):
    ...
    print(f"[OK] Biểu đồ đã lưu: {output_file}")
    
    plt.show()

def main():
    print("\n" + "=" * 70)
    print("SCATTER PLOT - Word Count Analysis")
    print("=" * 70 + "\n")
    
    documents = get_data()
    
    if documents is None or len(documents) == 0:
        print("[FAIL] Không có dữ liệu")
        return
    
    create_scatter_chart(documents)
