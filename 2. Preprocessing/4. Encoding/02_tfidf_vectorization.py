...
print(f"[OK] Total documents in collection: {total_docs:,}")
...
for i, doc in enumerate(collection.find({}), 1):
    ...
    # [WARN]️ IMPORTANT: tokens are already separated, join with space
...
print(f"\n[OK] Extracted {len(texts_list):,} documents")
...
print(f"[OK] TfidfVectorizer config:")
...
print(f"[OK] Fit complete in {fit_time:.2f} seconds")
...
print(f"[OK] Transform complete in {transform_time:.2f} seconds")
...
print(f"[OK] Concatenation complete in {concat_time:.2f} seconds")
...
if X_combined.shape == (total_docs, expected_features):
    print(f"[OK] Check 1: X matrix shape ({total_docs}, {expected_features})")
    checks_passed += 1
else:
    print(f"[FAIL] Check 1: FAILED - X matrix shape {X_combined.shape}")

# Check 2: Y shape
checks_total += 1
if y.shape == (total_docs,):
    print(f"[OK] Check 2: y labels shape ({total_docs},)")
    checks_passed += 1
else:
    print(f"[FAIL] Check 2: FAILED - y labels shape {y.shape}")

# Check 3: No NaN values
checks_total += 1
if np.isnan(X_combined.data).sum() == 0:
    print(f"[OK] Check 3: No NaN values in X")
    checks_passed += 1
else:
    print(f"[FAIL] Check 3: FAILED - Found NaN values")

# Check 4: site_onehot exactly one 1 per row
checks_total += 1
site_sum = site_array.sum(axis=1)
if np.all(site_sum == 1):
    print(f"[OK] Check 4: site_onehot has exactly one 1 per row")
    checks_passed += 1
else:
    print(f"[FAIL] Check 4: FAILED - site_onehot invalid")

# Check 5: Labels are valid
checks_total += 1
valid_labels = np.all((y >= -1) & (y <= 6))
if valid_labels:
    print(f"[OK] Check 5: All labels in valid range (-1 to 6)")
    checks_passed += 1
else:
    print(f"[FAIL] Check 5: FAILED - Invalid labels found")

# Check 6: Labeled/Unlabeled split
checks_total += 1
if labeled_count + unlabeled_count == total_docs:
    print(f"[OK] Check 6: Labeled/Unlabeled split ({labeled_count}/{unlabeled_count}) equals total docs")
    checks_passed += 1
else:
    print(f"[FAIL] Check 6: FAILED - Unexpected split ({labeled_count}/{unlabeled_count})")

print(f"\n[OK] Quality checks: {checks_passed}/{checks_total} PASSED")
if checks_passed < checks_total:
    print("[WARN]️  WARNING: Some checks failed")
...
try:
    ...
    print(f"[OK] Saved vectorizer.pkl ({vectorizer_size:.1f} MB)")
except Exception as e:
    print(f"[FAIL] Failed to save vectorizer.pkl: {e}")
...
try:
    ...
    print(f"[OK] Saved X_matrix.pkl ({X_size:.1f} MB, sparse format)")
except Exception as e:
    print(f"[FAIL] Failed to save X_matrix.pkl: {e}")
...
try:
    ...
    print(f"[OK] Saved y_labels.pkl ({y_size:.1f} MB)")
except Exception as e:
    print(f"[FAIL] Failed to save y_labels.pkl: {e}")
...
try:
    ...
    print(f"[OK] Saved vectorization_stats.json")
except Exception as e:
    print(f"[FAIL] Failed to save vectorization_stats.json: {e}")
...
Input:
  [OK] {total_docs:,} documents from MongoDB
  [OK] full_text_tokens extracted
  [OK] site_onehot extracted
  [OK] category_encoded extracted

TF-IDF Configuration:
  [OK] max_features: 5,000
  [OK] ngram_range: (1, 2) - Unigrams + Bigrams
  [OK] min_df: 2, max_df: 0.8
  [OK] sublinear_tf: True
  [OK] norm: L2

Output:
  [OK] X_matrix: ({total_docs:,} × {expected_features:,}) sparse matrix
    - 5,000 TF-IDF features
    - {site_features} site_onehot features
    - Sparsity: {stats['output']['sparsity_percent']:.1f}%
  
  [OK] y_labels: ({total_docs:,},) array
    - Labeled: {labeled_count:,} ({labeled_count/total_docs*100:.1f}%)
    - Unlabeled: {unlabeled_count:,} ({unlabeled_count/total_docs*100:.1f}%)
  
  [OK] vectorizer.pkl: Saved (reuse for inference)
  [OK] vectorization_stats.json: Saved (metadata)
...
Quality Checks: {checks_passed}/{checks_total} PASSED [OK]
