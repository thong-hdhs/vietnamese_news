...
print(f"\n[OK] Total processed: {total_processed:,}")
print(f"[OK] Total updated: {total_updated:,}")
if total_errors > 0:
    print(f"[FAIL] Total errors: {total_errors}")
...
print(f"""
[OK] PHASE 5.1 LABEL ENCODING COMPLETE

Fields added to MongoDB:
  [OK] site_onehot (array of 5 integers: 0 or 1)
  [OK] category_encoded (integer: 0-6 or -1 for unlabeled)

Mapping files saved:
  [OK] site_mapping.json ({len(site_mapping)} items)
  [OK] category_mapping.json ({len(category_mapping)} items)

Data summary:
  [OK] Total documents: {total_docs:,}
  [OK] Labeled: {labeled_count:,} (25.0%)
  [OK] Unlabeled: {unlabeled_count:,} (75.0%)
  [OK] Unique sites: {len(unique_sites)}
  [OK] Unique categories: {len(unique_categories)}
