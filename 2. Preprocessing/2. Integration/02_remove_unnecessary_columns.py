...
def verify_schema(db, collection_name):
    ...
    if sample:
        ...
        for field in sorted(sample.keys()):
            ...
            if isinstance(value, str):
                if len(value) > 100:
                    logger.info(f"   [OK] {field} ({value_type}): {len(value)} chars")
                else:
                    logger.info(f"   [OK] {field} ({value_type}): {repr(value)}")
            else:
                logger.info(f"   [OK] {field} ({value_type})")
    ...
    for field in FIELDS_TO_REMOVE:
        ...
        if count > 0:
            logger.warning(f"   [WARN]️ Field '{field}' still exists in {count} documents")
            docs_with_removed_fields += count
        else:
            logger.info(f"   [OK] Field '{field}' successfully removed")
    ...
    for field in FIELDS_TO_KEEP:
        ...
        if count == collection.count_documents({}):
            logger.info(f"   [OK] Field '{field}' found in all {count} documents")
        else:
            logger.warning(f"   [WARN]️ Field '{field}' found in only {count} documents")
