...
def connect_mongodb(connection_string):
    ...
    try:
        ...
        logger.error(f"[FAIL] Cannot connect to MongoDB: {e}")
        raise
...
def handle_invalid_data_delete(db, collection_name):
    ...
    logger.info(f"[CHART] Total documents before: {total_docs_before}")
    ...
    logger.info(f"\n[OK] Analysis complete! Found {stats['total_deleted']} invalid articles")
    ...
    if invalid_ids:
        ...
        if object_ids:
            try:
                ...
                logger.info(f"[OK] Deleted {result.deleted_count} articles from database")
            except Exception as e:
                logger.error(f"[FAIL] Error deleting documents: {e}")
        ...
        logger.info("[OK] No invalid articles found - nothing to delete")
    
    # Verify after deletion
    total_docs_after = collection.count_documents({})
    stats['total_after'] = total_docs_after
    
    logger.info(f"\n[CHART] Total documents after: {total_docs_after}")
    logger.info(f"[CHART] Removed: {total_docs_before - total_docs_after} articles")
...
def validate_remaining_articles(db, collection_name):
    ...
    if validation_stats['missing_title'] > 0:
        logger.warning(f"   [WARN]️  Missing title: {validation_stats['missing_title']} (UNEXPECTED!)")
    if validation_stats['missing_content'] > 0:
        logger.warning(f"   [WARN]️  Missing content: {validation_stats['missing_content']} (UNEXPECTED!)")
    if validation_stats['missing_source'] > 0:
        logger.warning(f"   [WARN]️  Missing source: {validation_stats['missing_source']} (UNEXPECTED!)")
...
def print_detailed_report(stats, validation_stats):
    ...
    logger.info(f"\n[CHART] VALIDATION RESULTS (After Deletion):")
    ...
    if stats['total_deleted'] > 0:
        logger.info(f"\n[OK] SAMPLE DELETED ARTICLES (first 10):")
...
def main():
    ...
    try:
        ...
        logger.info(f"\n[OK] Ready for next step: Detect & Remove Outliers")
        
    except Exception as e:
        logger.error(f"[FAIL] Script failed: {e}")
