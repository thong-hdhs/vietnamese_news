...
def detect_outliers_and_duplicates(db, collection_name):
    ...
    logger.info(f"[CHART] Total documents: {total_docs}")
    ...
    logger.info(f"\n[OK] Word count analysis complete!")
    ...
    logger.info(f"[OK] Duplicate detection complete!")
...
def delete_outliers_and_duplicates(collection, ids_to_remove):
    ...
    if ids_to_remove:
        try:
            ...
            logger.info(f"[OK] Deleted {result.deleted_count} documents!")
            return result.deleted_count
        except Exception as e:
            logger.error(f"[FAIL] Error deleting documents: {e}")
            raise
    else:
        logger.info("[OK] No documents to delete")
        return 0
...
def print_detailed_report(stats):
    ...
    logger.info(f"\n[CHART] WORD COUNT DISTRIBUTION (BEFORE):")
