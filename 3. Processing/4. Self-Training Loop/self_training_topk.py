...
def main():
    ...
    for iteration in range(1, MAX_ITERATIONS + 1):
        ...
        if val_f1_new > best_val_f1:
            ...
            print(f"      [OK] New best model! F1: {best_val_f1:.4f}")
    ...
    print(f"  [OK] Saved final model to: {FINAL_MODEL_FILE}")
    ...
    print(f"  [OK] Saved iteration log to: {SELF_TRAINING_LOG_FILE}")
    
    # Save pseudo labels
    with open(PSEUDO_LABELS_FILE, 'w', encoding='utf-8') as f:
        json.dump(pseudo_labels_all, f, ensure_ascii=False, indent=2)
    print(f"  [OK] Saved pseudo labels to: {PSEUDO_LABELS_FILE}")
