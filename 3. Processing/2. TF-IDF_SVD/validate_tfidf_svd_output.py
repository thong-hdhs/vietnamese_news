...
def main():
    ...
    print(f"  [OK] train_data.json: {len(train_rows)} rows")
    print(f"  [OK] val_data.json: {len(val_rows)} rows")
    print(f"  [OK] unlabeled_data.json: {len(unlabeled_rows)} rows")
    ...
    if explained_var < 0.5:
        ...
        print(f"  [OK] Good: Explained variance > 80%")
    else:
        print(f"  [WARN] Acceptable: Explained variance between 50-80%")
    ...
    if all_passed and len(warnings) == 0:
        print("\n[OK] ALL CHECKS PASSED - Output is ready for next phase")
        print("\nRecommendations:")
        ...
        if not all_passed:
            print("\n[FAIL] CRITICAL ERRORS - Do not proceed to next phase")
            print("   Please fix the issues above before continuing")
        else:
            print("\n[WARN]️  WARNINGS ONLY - Can proceed with caution")
